...
{
    ASSERT1(entity != NULL)
    GameObject *game_object = DStaticCast<GameObject *>(entity);

    // add it to the master entity set
    m_entity_set.insert(game_object);

    // if it's not CT_NO_COLLISION, add its
    // owner object into the collision quadtree
    ASSERT1(m_main_object_layer != NULL)
    if (game_object->GetCollisionType() != CT_NO_COLLISION)
    {
        DEBUG1_CODE(bool add_was_successful =)
        m_quad_tree->AddObject(game_object->GetOwnerObject());
    ...
    Engine2::Entity *const entity)
{
    ...
    m_entity_set.erase(game_object);

    // if it's not CT_NO_COLLISION, remove its
    // owner object from the collision quadtree
    if (game_object->GetCollisionType() != CT_NO_COLLISION)
        game_object->GetOwnerQuadTree(Engine2::QTT_PHYSICS_HANDLER)->RemoveObject(game_object->GetOwnerObject());
}
