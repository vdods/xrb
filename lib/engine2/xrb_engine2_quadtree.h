...
namespace Engine2
{
    ...
    public:
        ...
        inline QuadTreeType GetType () const
        {
            return m_type;
        }
        inline QuadTree *GetParent () const
        {
            return m_parent;
        }
        inline bool GetHasChildren () const
        {
            return m_child[0] != NULL;
        }
        // returns the root node (top level parent) of this quadtree node
        QuadTree const *GetRootNode () const;
        inline FloatVector2 const &GetCenter () const
        {
            return m_center;
        }
        inline Float GetSideLength () const
        {
            return 2.0f * m_half_side_length;
        }
        inline Float GetHalfSideLength () const
        {
            return m_half_side_length;
        }
        inline Float GetRadius () const
        {
            return m_radius;
        }
        inline Uint32 GetSubordinateObjectCount () const
        {
            return m_subordinate_object_count;
        }
        inline Uint32 GetSubordinateNonEntityCount () const
        {
            return m_subordinate_non_entity_count;
        }
        ...
        inline bool GetIsAllowableObjectRadius (Object const *const object) const
        {
            return object->GetRadius() / m_radius > 0.5f;
        }
    ...
    protected:
        ...
        // increment m_subordinate_non_entity_count at this node and up through all parents
        void IncrementSubordinateNonEntityCount ();
        // decrement m_subordinate_object_count at this node and up through all parents
        void DecrementSubordinateObjectCount ();
        // decrement m_subordinate_non_entity_count at this node and up through all parents
        void DecrementSubordinateNonEntityCount ();
    ...
    private:
        ...
        Uint32 m_subordinate_non_entity_count;
