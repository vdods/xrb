...
void LoadSvgIntoWorld (std::string const &svg_path, World &world, Float current_time)
{
        ...
        svg->NextElement(g_it, g, "g"), ++layer_number)
    {
        ...
        if (g->AttributeValue("ignore") == "true")
            continue;
        ...
        // create a matrix to represent the transformation taking the left-handed coordinate
        // system used by SVG to the right-handed one used by XRB.
        FloatMatrix2 outer_coordinate_transform(
            1.0f,  0.0f, bounding_box_half_side_length,
            0.0f, -1.0f, bounding_box_half_side_length);

        // read x/y offset for bounding box (this needs to be used to transform
        // the x/y post-translate coordinates of subordinate images)
...
//             Float square_size = 0.5f * (width + height);
            ...
            else if (transform_string.find("matrix(") == 0)
            {
                ...
                if (angle_variance > 0.01f)
                    THROW_STRING("skew transformation detected (no skew transformation allowed, because then the sprite angle is undefined)");
            }
            else
            ...
            // optional attributes which specify entity properties
            ...
            // if the entity type is nonempty, create an Entity
            Entity *entity = NULL;
            ...
            if (entity != NULL)
                ...
                fprintf(stderr, "LoadSvgIntoWorld(\"%s\"); in <image id='%s'>, layer '%s': %s\n", svg_path.c_str(), image_id.c_str(), layer_id.c_str(), exception.c_str());
            }
        }
