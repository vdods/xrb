...
Uint32 MapEditor2::VisibilityQuadTree::DrawMetrics (
    ...
    Object::MetricMode const metric_mode)
{
    ...
    if (!GetDoesAreaOverlapQuadBounds(
            draw_data.GetViewCenter(),
            draw_data.GetViewRadius()))
    {
        return retval;
    }
    ...
    if (GetHasChildren())
        for (Uint8 i = 0; i < 4; ++i)
            retval += GetChild<VisibilityQuadTree>(i)->DrawMetrics(draw_data, metric_mode);

    return retval;
}

Uint32 MapEditor2::VisibilityQuadTree::DrawMetricsWrapped (
    ...
    for (Float x = left; x <= right; x += 1.0f)
    {
        for (Float y = bottom; y <= top; y += 1.0f)
        {
            ...
            if (view_offset.GetLengthSquared() < radius_sum*radius_sum)
            {
                ...
                retval += DrawMetrics(draw_data, metric_mode);
            }
        }
    }
